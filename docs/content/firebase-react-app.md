...
internally within ScreenCloud. The application uses 15Fives's api to show High Five's that employees receive from their peers within the company. 
...
written using Javascript and ReactJs. This application is loaded on screen using our ScreenCloud player. You can think of this as a browser that loads the application
...
fetches its data periodically from the third party api, which in this case is 15Five. From the diagram, you can see that we do not call the api directly from the web app on screen,
instead we call a firebase cloud function which in turn calls the api endpoint for the data. We do this because third party api's generally require
an api key or access token which allows you to be able to use their api and for security reasons this should never be exposed on the frontend. Firebase allows us to store this
key/value in its configuration store securely and to be used by the cloud function for each call. We will show you how this is done later in the tutorial.

## Getting started with Firebase

We will be hosting the web application using Firebase. We will also be creating a Firebase cloud function to fetch the data from the 15Five api. 

To get started we create a firebase account and then set up a project for our application. We will call it screencloud-15five. We create the account and project on [Firebase](https://firebase.google.com/) 

Just follow the instructions to create the Firebase account and create the empty project. Just create an empty project there is no need to add a web application or anything else through the Firebase console.
Later in the tutorial we will show you how to add a web application and cloud function to the firebase project through the command line. 
...
1. create a folder that will hold your cloud function code
```shell
...
firebase login
```
4. create firebase cloud function - follow instructions shown on screen to link the cloud function to your firebase project
```shell
...
The code we added to the index.js file for the cloud function can be found on [https://github.com/screencloud/developer/tree/master/examples/15five-firebase-cloud-function-example](https://github.com/screencloud/developer/tree/master/examples/15five-firebase-cloud-function-example)  
...
You will have to add your api key to the code replacing the TOKEN value with that of your own. For now just hardcode it, but later when we talk about deploying the function we will show you how to add the value to the Firebase configuration store meaning it will
...
The app which shows the data on screen is a simple web application , we will be creating the application using Javascript and React, 
...
This will create the base code for your React App. We are creating a 'Slide Show' style application, a very common type of app used to display information
on screens. We have put together an example application which, with a few changes, can help create your own bespoke 'Slide Show' style app using your own data. The code for this can be found on:
